...
-- ─────────────────────────────────────────── добавлено для AGLoader
--
-- Оригинал умеет разбирать адрес, но не собирать строку запроса, а она
-- нужна почти каждому скрипту. Две функции ниже — единственное отличие
-- этого файла от апстримного.

-- Таблица в строку запроса. Ключи сортируются, поэтому одни и те же данные
-- всегда дают одну и ту же строку.
function _M.buildquery(t)
  ...
  for _, k in ipairs(keys) do
    parts[#parts + 1] = _M.escape(k) .. '=' .. _M.escape(tostring(t[k]))
  end
  return table.concat(parts, '&')
end

-- Строка запроса обратно в таблицу.
function _M.parsequery(s)
  local t = {}
  for pair in tostring(s or ''):gmatch('[^&]+') do
    local k, v = pair:match('^([^=]*)=?(.*)$')
    if k and k ~= '' then t[_M.unescape(k)] = _M.unescape(v) end
  end
  return t
end
