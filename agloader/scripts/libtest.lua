...
local function runLocal()
  ...
  check('движок', 'камера движка', function()
    ...
    local m = assert(ag.viewMatrix(), 'матрица вида не читается')
    local px = assert(ag.cameraPose(), 'кадр камеры не читается')
    return ('0x%X, поле зрения %.0f°'):format(c, ag.fov() or 0)
  end)
