...
-- Поэтому час с минутой не «ставятся», а удерживаются — переписываются
-- заново десять раз в секунду, пока держание включено.
local hold = false
...
function main()
  ...
  -- Держим час и минуту, пока включено. Десять раз в секунду хватает, чтобы
  -- подмена была незаметна, и это два байта записи — не нагрузка.
  while true do
    if hold then
      local h = ag.time()
      if h then ag.setTime(holdH, holdM) end
    end
    wait(100)
  end
end
