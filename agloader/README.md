...
Разбор `libag-client.so` (arm64, v17.7.1) — перекрёстные ссылки ADRP+ADD по
`.text`. Смещения от базы библиотеки:

| Смещение | Что это |
|---|---|
| `+0x35E5028` | `bool`: включает встроенный в движок редактор `Timecycle editor` |
| `+0x35E5024` | `int32`: выбранный слот редактора, 0..7 (00:00, 03:00 … 21:00) |
| `+0x3CCA720` | указатель на главный синглтон движка (384 ссылки в коде) |

Редактор таймцикла — родное отладочное окно игры на ImGui, просто выключенное
флагом: освещение, туман, небо и постобработка по восьми точкам суток. Найден
так: во всём движке у `ImGui::Begin` (`0x8dee00`) всего три вызова, и
единственный не-внутренний — `Begin("Timecycle editor", &флаг)`, причём тот же
адрес проверяется через `ldrb`+`cbz` прямо перед вызовом. Включается из
`st_weather.lua`, вкладка «Движок», или командой `/tcedit`.

Часы и погода **не найдены**. Движок держит их не в `.bss`, а в куче внутри
синглтона: обращения идут через загрузку указателя и смещения за 15000, так
что статически адрес не вычислить — нужен поиск по значению на живой игре.
Смещение поля внутри синглтона стабильно между запусками, в отличие от самого
адреса, поэтому вкладка «Поиск» сохраняет именно его.
