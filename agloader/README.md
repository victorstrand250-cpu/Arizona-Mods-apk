...
* ImGui-оверлей поверх игры с плавающей кнопкой вызова (её можно
  перетаскивать), встроенным менеджером скриптов, консолью лога и списком
  модулей.
...
├── scripts/   *.lua — грузятся при старте
├── lib/       Lua-модули для require и .so для package.cpath
├── config/
└── logs/agloader.log
```

Каталог `Android/media` пишется обычным файловым менеджером без рута и без
особых разрешений. Загрузчик создаёт его сам при первом запуске; примеры
из `scripts/` можно просто скопировать туда.
