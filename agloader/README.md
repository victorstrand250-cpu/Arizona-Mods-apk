...
Полей ввода это тоже касается. Поднимается **та же клавиатура, что и под
чатом игры**: у `GTASA` есть публичный `SetInputLayout(int, boolean)`,
который сам перекладывает вызов на UI-поток, а объект активити лежит в
статическом поле `_instance`. Напечатанное приходит в `OnInputEnd`, который
загрузчик и так перехватывает, — остаётся не пустить текст в чат и отдать
его полю. Если этих методов почему-то нет, включается запасная клавиатура,
нарисованная кнопками ImGui.
