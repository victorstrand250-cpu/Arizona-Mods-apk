...
Камера в коде не находится: движок пишет вызовы GL в командный буфер, и
указатель на матрицы в аргументах — копия внутри буфера. Зато она
опознаётся на живой игре по форме данных — это единственная матрица
положения в нескольких метрах от игрока, смотрящая прямо на него. Этим
занимается `/recon`, кнопка «Найти камеру».

Часы и погода пока не найдены: движок держит их в куче внутри синглтона.
Ищутся поиском по значению — `/recon` умеет и это.
...
| `03_test.lua` | `/test` | по очереди проверяет всё, что умеет сам загрузчик |
| `speedometer.lua` | `/speed` | скорость, координаты, пройденный путь — самая короткая проверка чтения памяти |
| `playeresp.lua` | `/pesp` | метки над игроками: массив слотов, пакетное чтение и проекция камеры разом |
...
| `netprobe.lua` | `/netinfo` | без окна: какие модули поднялись, `/tcping`, `/dns`, `/geturl` |
| `autobind.lua` | `/bind` | без окна: макросы в чат с паузами и подстановкой координат |
| `waypoints.lua` | `/wp` | без окна: точки на карте, расстояние и направление до них |
