...
## Что сделано вместо порта: AGLoader

Порт MonetLoader упирается в реверс закрытого движка и не окупается. Но
этапы 1–3 из плана выше не обязательно проходить целиком, чтобы получить
рабочий загрузчик: якорей `androidStep` / `androidMultiTouch` /
`androidResize` достаточно для кадра, ввода и оверлея, а произвольный
доступ к памяти даёт всё остальное.

Поэтому в каталоге [`../agloader`](../agloader) лежит **AGLoader** —
отдельный Lua-загрузчик под новый движок:

* перехват через `RegisterNatives` (не инлайн-хук) — не зависит ни от
  порядка загрузки библиотек, ни от версии ART, ни от перестановок кода
  внутри `libag-client.so` при LTO+BOLT;
* LuaJIT 2.1, по `lua_State` на скрипт, модель MoonLoader
  (`main()` в корутине, `wait`, `onFrame` / `onImgui` / `onTouch` / …);
* ImGui-оверлей поверх игры, менеджер скриптов, консоль лога;
* `memory.*` с безопасным чтением через `process_vm_readv` и поиском по
  сигнатурам `memory.scan`, плюс `ffi` из LuaJIT.

Чего в нём нет и не может быть: `game.*`, опкодов, `sampev`/`sampfuncs`.
Причины те же, что описаны выше, — этих подсистем в новом движке
физически нет. Существующие скрипты MonetLoader/MoonLoader на нём
не заработают: у AGLoader свой API.

Подробности, сборка и описание Lua API — в
[`../agloader/README.md`](../agloader/README.md).
...
3. Загрузчик под новый движок построить **можно**, и он построен
   (`agloader/`) — но на других якорях и с другим API.
4. Если нужны именно старые скрипты MonetLoader — работает только старая
   сборка на старом движке, она в релизе `v1.0`.

## Ссылки

* [`../agloader/README.md`](../agloader/README.md) — AGLoader: устройство,
  сборка, Lua API
